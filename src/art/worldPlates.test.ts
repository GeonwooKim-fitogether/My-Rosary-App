 ...
 * 서명·워터마크가 있는 그림.
 *
 * 2026-09-17 에 열일곱 장의 오른쪽 아래를 모두 눈으로 확인해 넷을 찾았다. 셋은 같은
 * 스튜디오의 워터마크이고 하나는 그림 안에 그려진 서명이다. 표에서 빠져 있는 한 장
 * (`09-color-jesus.jpg`)은 스톡 사진 워터마크가 화면 전체에 반복돼 시안이 이미 뺐다.
 *
 * 이 시험은 그 넷을 붙들어 둔다. 정식 자산으로 교체할 때(D-1) 무엇을 교체해야 하는지가
 * 이 목록이고, 목록이 줄거나 늘면 시험이 먼저 알려 준다.
 */
describe('작가 서명·워터마크가 있는 그림을 놓치지 않는다 (D-1 · Q-50)', () => {
  it('넷이고, 그 넷이 이것들이다', () => {
    expect([...MARKED_PLATES].sort()).toEqual(['02', '04', '11', '13']);
  });

 ...
 * | 중심 메달 | 시작 기도의 영광송 | — (알이 아니다) |
 * | 고리의 큰 알 다섯 | 각 단의 신비 선포·주님의 기도 | 5 |
 * | 고리의 작은 알 쉰 | 각 단의 성모송 열 | 50 |
 *
 * 합이 쉰아홉이고 이것이 실제 묵주의 알 수다.
 *
 * **알에 머물지 않는 단계를 어디에 두었나.** 성호경과 사도신경은 십자가에서 바치는
 * 기도이므로 십자가가 빛나고, 시작 기도의 영광송은 세 알을 지나 고리로 들어서는 자리인
 * 중심 메달에서 빛난다. 각 단의 영광송과 구원을 비는 기도는 **그 단의 열째 알에 머문다** —
 * 실제로 그 두 기도는 열째 알을 쥔 채 바치고, 다음 큰 알로 손을 옮기는 것은 다음 단을
 * 시작할 때이기 때문이다. 앞으로 나아간 척하지 않는 쪽이 "지금 어디까지 왔나"에 정직하다.
 *
...
/**
 * 타원 위의 한 점. 매개변수 t 가 0 이면 맨 아래(메달 자리)이고, t 가 커지면 왼쪽 →
 * 위 → 오른쪽 순으로 돈다. 기도가 왼쪽에서 시작해 오른쪽으로 흐르게 하려는 것이고,
 * 이것은 v5 가 열 알을 왼쪽에서 오른쪽으로 늘어놓은 방향과 같다.
 */
...
export function rosaryStateFor(step: {
  ...
  if (step.section === 'opening') {
    ...
    // 성호경·사도신경은 십자가에서, 영광송은 세 알을 지나 선 중심 메달에서 바친다.
    if (step.prayer === 'glory') return { done: PENDANT_BEADS, current: -1, focus: 'medal', label: null };

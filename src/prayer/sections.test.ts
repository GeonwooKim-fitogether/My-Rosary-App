 ...
 * 가는 자리는 언제나 **그 구간의 첫 단계**(단이면 신비 선포)다. 셋째, 양 끝에서는
 * 갈 데가 없다고 정직하게 답한다 — 있지도 않은 제6단으로 뛰지 않는다.
 */
import { mysteryForFiftyfourDay } from '../domain/mysteries';
import { sectionLabel, sectionMoves, sectionOf, sectionStart, sectionsOf } from './sections';
import { buildDayQueue } from './steps';

const QUEUE = buildDayQueue(mysteryForFiftyfourDay(23));

describe('구간 나누기', () => {
  it('하루는 시작 기도와 다섯 단, 모두 여섯 구간이다', () => {
    expect(sectionsOf(QUEUE)).toEqual([0, 1, 2, 3, 4, 5]);
  });

  it('구간 이름은 시작 기도와 제N단이다', () => {
    expect(sectionLabel(0)).toBe('시작 기도');
    expect(sectionLabel(3)).toBe('제3단');
  });

  it('구간의 첫 단계는 시작 기도의 성호경과 각 단의 신비 선포다', () => {
...
describe('앞 단·다음 단으로 옮길 곳', () => {
  it('시작 기도 어디에 서 있어도 다음은 제1단이고, 앞은 갈 데가 없다', () => {
    for (let at = 0; at < 7; at++) {
      ...
      expect(moves.next).toEqual({ section: 1, index: 7, label: '제1단' });
    }
  });
  ...
  it('제5단에서는 다음이 없다 — 없는 단으로 뛰거나 하루를 끝내지 않는다', () => {
    const last = QUEUE.length - 1;
    const moves = sectionMoves(QUEUE, last);
    expect(moves.next).toBeNull();
    expect(moves.previous?.label).toBe('제4단');
  });

  it('구간을 여섯 번 옮기면 시작 기도에서 제5단까지 한 칸씩 간다', () => {
    let at = 0;
    ...
    expect(visited).toEqual(['시작 기도', '제1단', '제2단', '제3단', '제4단', '제5단']);
  });
});

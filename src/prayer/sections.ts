/**
 * 구간 — 하루 77단계를 사람이 옮겨 다닐 수 있는 여섯 덩어리로 본다.
 *
 ...
 * | 구간 번호 | 이름 | 단계 |
 * |---|---|---|
 * | 0 | 시작 기도 | 0~6 (성호경 · 사도신경 · 주님의 기도 · 성모송 셋 · 영광송) |
...
/** 구간 하나. 0 은 시작 기도, 1~5 는 그 번호의 단이다. */
export type SectionNumber = number;

/** 구간을 가진 단계라면 어느 구간인가. */
export function sectionOf(step: { section: string; decade?: number | null }): SectionNumber {
  return step.section === 'decade' ? (step.decade ?? 1) : 0;
}

/** 화면에 적는 구간 이름. 형식은 v5 의 구간 라벨과 같다. */
export function sectionLabel(section: SectionNumber): string {
  return section === 0 ? '시작 기도' : `제${section}단`;
}

/**
 * 그 구간이 시작되는 단계의 자리.
 *
 * **구간의 첫 단계로 간다** — 제3단이면 신비 선포다. 주님의 기도가 아니라 신비 선포를
 * 고른 이유는 둘이다. 첫째, 77단계에서 실제로 그 단의 첫 단계가 신비 선포다. 둘째,
 * 단을 건너뛴 사람에게 가장 먼저 필요한 것이 "지금 무엇을 묵상하는가"이고 그것을
 * 말해 주는 단계가 신비 선포다. 뛰어든 자리에서 묵상할 것을 모른 채 주님의 기도부터
 * 시작하면, 건너뛰기가 기도를 탐색으로 만들어 버린다.
 *

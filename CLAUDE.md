...
**관문 1(기획안 컨펌) 대기.** 코드는 아직 없다. 진행 순서는 기획안 컨펌 → KimDesigner 시안 컨펌(관문 2) → 개발(M0~M5)이다. 현재 상태와 다음 할 일은 언제나 `decisions.md`와 `docs/plan/development-plan.md`가 정본이다 — 이 파일의 이 절은 요약일 뿐이니, 어긋나면 그 두 파일을 믿는다.

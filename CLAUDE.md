...
| **W3 슬라이스 A 에서 찍은 화면 사진** | `docs/plan/w3-screens/` — 새 시안의 어법으로 다시 세운 여정 화면 넉 장(접힌 목록 · 펴진 줄을 390 과 320 두 너비에서 · 여정이 없는 화면)과 여정 완주 화면 한 장. 날짜 격자를 아홉 열로 통일한 판단을 두 너비의 사진으로 확인할 수 있고, 이 폴더가 대신하는 옛 사진 셋은 그 폴더의 `README.md` 가 적는다 |
| **W2 슬라이스 A 에서 찍은 화면 사진** | `docs/plan/w2-screens/` — 새 시안의 어법으로 다시 세운 홈 넉 장(여정이 선 홈 · 빈 홈 · 이어서 바치는 홈 · `다시 바치기` 확인 시트)과 성화 갤러리 자리 한 장. 시안과 달라 보이는 다섯 자리의 이유와 아직 옛 어법으로 남은 자리는 그 폴더의 `README.md` |

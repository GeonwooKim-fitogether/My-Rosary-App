...
| **W3 에서 찍은 화면 사진** | `docs/plan/w3-screens/` — 열 장이다. 슬라이스 A 의 다섯 장은 새 시안의 어법으로 다시 세운 여정 화면 넉 장(접힌 목록 · 펴진 줄을 390 과 320 두 너비에서 · 여정이 없는 화면)과 여정 완주 화면이고, 슬라이스 B 의 다섯 장은 성화 갤러리 석 장(지역 탭 · 모든 성화 탭 · 빈 즐겨찾기 탭)과 전체 화면 감상 두 장(껍데기가 보일 때와 사라졌을 때)이다. 날짜 격자를 아홉 열로 통일한 판단을 두 너비의 사진으로 확인할 수 있고, 이 폴더가 대신하는 옛 사진 셋과 성화의 워터마크는 그 폴더의 `README.md` 가 적는다 |
| **W2 에서 찍은 화면 사진** (`gallery.png` 는 **얼린 계보**) | `docs/plan/w2-screens/` — 새 시안의 어법으로 다시 세운 홈 넉 장(여정이 선 홈 · 빈 홈 · 이어서 바치는 홈 · `다시 바치기` 확인 시트)과 성화 갤러리 **자리 지킴이** 한 장. **`gallery.png` 은 2026-09-18 에 얼렸다** — W3 슬라이스 B 가 그 자리를 시안의 갤러리로 채웠고, 계속 찍으면 자리 지킴이의 기록이 덮이기 때문이다. 새 갤러리의 사진은 `docs/plan/w3-screens/` 에 있다. 홈 넉 장은 같은 날 **전체 화면으로 여는 단추**가 들어오며 다시 찍혔다. 시안과 달라 보이는 다섯 자리의 이유와 아직 옛 어법으로 남은 자리는 그 폴더의 `README.md` |

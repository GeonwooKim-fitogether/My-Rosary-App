 ...
 *
 * **세는 법을 먼저 적는다.** 여기에는 시안과 **실제로 다른 것만** 센다. 시안의 값을 그대로
 * 옮겼지만 설명이 필요한 것은 아래 "시안 그대로지만 설명이 필요한 값" 으로 내렸다 —
 * 차이가 아닌 것이 이 표에 섞이면 "임의로 바꾼 자리가 몇인가" 를 세는 사람이 부풀려 읽는다
 * (2026-09-18 KimDesigner 검수).
 *
 ...
 * 4. **탭 글자의 줄바꿈 금지가 시안보다 약하다.** 시안은 세 탭 모두에 `white-space:nowrap`
 *    을 걸어 줄바꿈을 아예 막는데, 이 저장소는 한국어 줄바꿈 규칙(`koWordBreak`, 낱말 안은
 *    못 끊고 띄어쓰기에서는 끊긴다)만 건다. `모든 성화` 에 띄어쓰기가 있으므로 **너비 320px
 *    에 앱 안 글자 크기가 `아주 크게` 인 조합에서는 두 줄이 되어 띠가 높아질 수 있다.**
 *    그 조합의 사진은 아직 없다 (2026-09-18 KimDesigner 검수가 계산으로 짚었다).
 ...
 * ── 시안 그대로지만 설명이 필요한 값 ────────────────────────────────────────────
 *
 * - **캡션 줄 오른쪽 여백이 음수다** (`margin:-10 -12 -10 0`). 13px 글자 한 줄 안에서
 *   하트의 누르는 자리를 44×44 로 지키려는 시안의 값 그대로다. 차이가 아니다.
 * - **고르지 않은 탭 글자가 본문과 같은 먹빛이다.** 한때 흐린 회색을 썼으나 시안 자신의
 *   렌더와 화소를 맞춰 보고 되돌렸다 (`decisions.md` Q-75). 지금은 시안과 같다.
 * - **작은 라벨과 고른 탭의 글자가 시안의 강조색보다 짙다**(`accentText`). 이것은 이 화면의
 *   판단이 아니라 저장소 전체의 규칙이다 — 시안의 강조색이 밝은 종이 위에서 대비 2.26~2.94
 *   밖에 나오지 않아, 「Classical」 자신이 적어 둔 "본문 크기 글자에는 더 짙은 단계를 쓰라"
 *   는 권고대로 되돌린 것이다 (`decisions.md` Q-51, 2026-09-18 에 닫혔다).
 */

 ...
 *
 * ── 문서의 언어(`lang`)는 여기서 정하지 않는다 — 앱이 켜진 뒤에 고쳐 쓴다 ────────────
 *
 ...
 *
 * **그 일은 W4 슬라이스 B 에서 실제로 됐다.** 이 껍데기의 기본값은 `en` 그대로이고, 앱이
 * 켜지는 순간과 언어가 바뀌는 순간마다 `src/i18n/documentLanguage.ts` 가 브라우저에서
 * `document.documentElement.lang` 을 실제 언어로 고쳐 쓴다(부르는 자리는 `app/_layout.tsx`
 * 의 `Shell`). 그래서 미리 찍어 둔 html 은 한 글자도 바뀌지 않은 채, 사람이 보는 화면의
 * 문서만 제 언어를 말하게 됐다.
 *
 * 다만 위에서 잰 사실은 그대로 남는다 — 그 한 줄이 도는 순간 크로미움이 글자 고르기와 줄
 * 끊기를 바꾸므로 **한국어 화면의 사진이 달라진다.** 슬라이스 B 가 다시 재어 서른 장이
 * 달라지는 것을 확인했고, 그 경위는 `docs/plan/w4-screens/README.md` 가 적는다.
 */

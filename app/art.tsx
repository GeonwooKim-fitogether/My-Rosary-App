 ...
 * 5. **그림 제목을 두 줄에서 자른다.** 시안은 줄 수를 제한하지 않는다. 한국어 제목은 가장 긴
 *    것도 한 줄에 들어가 지금은 차이가 드러나지 않지만, **영어 벌의 긴 제목**(예:
 *    `The Multitude Toward the Light`)은 20px 명조로 약 200px 폭에서 세 줄이 될 수 있고
 *    그러면 끝이 잘린다. 영어 화면의 사진이 아직 없어 눈으로 확인하지 못했다
 *    (2026-09-18 KimDesigner 검수).
 * 6. **그림이 나타날 때 서서히 켜지지 않는다.** 시안은 `animation: mrFade 700ms` 로 그림을
 *    페이드로 띄운다. 이 저장소는 곧바로 그린다 — 홈의 큰 그림도 이미 같은 상태이고
 *    (시안은 900ms), 동작 줄이기를 켠 사람에게는 시안에서도 사라지는 움직임이다.
 *    **W3 이 만든 차이가 아니라 W2 부터 이어져 온 자리**이며, 정지 사진으로는 보이지 않는다.
 */

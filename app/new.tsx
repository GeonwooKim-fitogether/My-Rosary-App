 ...
 *
 * ── 계보 — 이 화면으로 오는 길은 2026-09-18 에 끊겼다 (W3 슬라이스 A) ───────────────
 *
 * **여정을 만드는 자리가 여정 화면(`app/journey.tsx`) 하나로 모였다**
 * (`docs/plan/w3-work-order.md` §1-2). 새 시안이 여정 화면 아래쪽에 만드는 자리를 붙여
 * 두었고, 두 자리를 다 두면 사람이 "둘 중 어디서 만들어야 맞나"를 묻게 되며 둘의 결과가
 * 달라지면 그것이 곧 결함이기 때문이다.
 *
 * 이 파일을 가리키던 진입점 넷(`app/home.tsx` · `app/mystery.tsx` · `app/guide.tsx` ·
 * `app/all-done.tsx`)은 모두 `/journey` 로 돌렸다. **화면과 코드는 지우지 않고 진입점만
 * 끊었다** — 결정 12-A 가 로그인·초대 코드 화면을 다룬 방식과 같으며, 되살릴 때 그 넷 중
 * 하나를 이 파일로 되돌리면 된다.
 *
 * 옛 네 구역이 어디로 갔는지는 위 지시서 §1-2 의 표에 있다. 요약하면 지향은 여정 화면의
 * 만드는 자리에 그대로 있고, 청원·감사는 고르지 않고 FR-35 의 규칙이 정하며, 낭송과
 * 묵주는 W2 가 설정 화면으로 옮겼다.
 *

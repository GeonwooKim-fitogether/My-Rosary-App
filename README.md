...
| 단위 테스트 | `npm test` |
| 화면 e2e (웹 빌드를 브라우저로 열어 확인) | `npm run test:e2e:fresh` (빌드부터 다시 한다) |
| 이미 빌드해 둔 `dist/` 로만 e2e | `npm run test:e2e` |
...
### 폰에서 써 보기 — 계정 승인 전의 임시 통로

개발자 계정이 승인되기 전에는 폰에 앱을 설치할 수 없습니다. 그런데 이 앱이 답해야 하는 질문("잠자기 전에 손대지 않고 끝까지 가는가")은 폰에서만 답이 나옵니다. 그래서 웹 빌드를 **파일 하나짜리 HTML** 로 묶는 도구를 두었습니다.

```bash
npm run build:web
node tools/pack/pack-single-html.mjs      # dist-single/myrosary.html 이 나옵니다
```

성화 열여섯 장은 파일 안에 박고, 한국어 글꼴 세 벌(26MB)은 박지 않고 Google Fonts 에서 받도록 `@font-face` 를 다시 씁니다. 그래서 결과가 약 4MB 로 떨어져 어디에나 올릴 수 있습니다. 나온 파일을 웹에 올리면 폰의 브라우저에서 주소 하나로 열립니다.

**이것은 설치본이 아니라 그 앞에 두는 임시 통로입니다.** 확인되는 것은 화면·흐름·자동 진행·한국어 낭송까지이고, 아래 셋은 설치본에서만 확인됩니다.

| 웹 판에서 확인되지 않는 것 | 왜 |
|---|---|
| 아이폰의 진동 | iOS 사파리는 웹 페이지에 진동을 허용하지 않습니다. 안드로이드 크롬은 됩니다 |
| 이어폰 단추 | 앱이 "재생 중인 소리"를 잡아야 받을 수 있고, 그것은 설치본의 일입니다 |
| 흔들기 | iOS 는 페이지에 움직임 센서를 주기 전에 사용자에게 한 번 묻습니다. 그 허용 절차가 웹 판에는 없습니다 |

경위와 이 순서를 정한 이유는 [`decisions.md`](decisions.md) 결정 4 에 있습니다.

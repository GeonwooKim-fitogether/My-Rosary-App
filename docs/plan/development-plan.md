...
| 2 | N 새 기도 — 구조 | 구역 넷: ①지향 ②청원/감사 ③낭송 방식 인라인 ④묵주 고르기 | FR-33 세 걸음: 1/3 바람(24자) · 2/3 형식(54일·9일·날마다) + 혼자/함께 · 3/3 시작일 + 판본(S4) + **완료 예정일 자동 계산** + 첫날 신비 미리보기 | **PRD 따름** — 세 걸음으로 다시 그린다 (관문 2 과제) |
| 3 | N — 청원/감사 | 사용자가 고른다. "감사 = 쉰네 날 내내" | 고르지 않는다. 54일의 1~27일 청원, 28~54일 감사로 **자동**(FR-35). "쉰네 날 내내 감사"는 PRD에 없다 | **PRD 따름** — 선택 칸 삭제 |
| 4 | N — 낭송 방식 인라인 | "차별점 #2이므로 인라인 노출, 시트로 숨기지 말 것" | §3-5 2항·06-b §9 23번: 고르는 일은 설정에서만. 차별점 2는 "첫 성모송에서 겪게 한다" | **공방장 결정 필요 (Q-07)** — 추천 PRD 따름 |
...
| 8 | 초대 코드 입력 | 별도 화면 "2. 초대 코드 입력"(여섯 칸 + 여정 미리보기 카드) | 06-e 결함 12 처방: 홈의 **요소 하나**(코드 입력). 새 화면이 아니다 | **공방장 결정 필요 (Q-02)** — 추천 PRD 따름: 홈 위 시트로, 여섯 칸 UI는 살림 |
| 9 | L 로그인 — 초대 코드 진입 | 로그인 화면에 `초대 코드로 들어가기` | 계정 필수(FR-32). 코드 입력은 홈에서(FR-40) — 로그인 전 합류는 성립하지 않는다 | **PRD 따름** — L에서 삭제 |
| 10 | "함께 바치기 배정" 화면 + "다섯 단 모두 혼자 바치기" | 새 화면과 단추 | 화면 목록 밖. 조 배정은 홈 카드 넷째 줄과 D의 조 영역(FR-40). "혼자 다 바침"은 §9가 V1.5로 미룬 다른 조 방식 | **공방장 결정 필요 (Q-08)** — 추천 PRD 따름: 만들지 않음 |
...
**넣는 것**: PRD §4 FR-01~45 전부(FR-08·09 결번, FR-29 폐기 제외). 화면 여덟(L·A·N·B·C·C′·D·E) + 시트 일곱. 밤·낮 두 벌. 양 플랫폼.
...
KimDesigner가 프로토타입 "한지 낮" 벌을 출발점으로 §8의 과제를 수행해 **화면 여덟 + 시트 일곱 × 두 벌(밤·낮)** 을 확정한다. 산출물은 프로토타입과 같은 형식(브라우저에서 실제 크기로 렌더되는 HTML 시안 + README 명세)이고, 06-b §9 수용 기준 24항목 자가 점검표를 붙인다. 공방장이 컨펌하면 개발이 시작된다. **컨펌 기준**: §3 정합 확인표의 "PRD 따름" 항목이 전부 반영됐고, "공방장 결정 필요" 항목은 관문 1의 답대로 그려졌으며, 24번("Marcello보다 세련")은 공방장 눈으로 판정.
...
| # | 과제 | 근거 | 산출물 |
|---|---|---|---|
| D-A | **밤 벌** — 쪽빛 `#10161F` 바탕의 두 번째 벌을 낮 벌과 동등 품질로 | 06-b §2-2, D-2 | 화면 여덟 + 시트 일곱 밤 벌 |
| D-B | **N 새 기도를 세 걸음으로** 다시 그림 — 1/3 바람(24자) · 2/3 형식 + 혼자/함께 · 3/3 시작일 + 판본 + 완료 예정일 + 첫날 신비. 청원/감사 선택 삭제, 묵주 고르기 삭제. 낭송 방식은 Q-07 답대로 | FR-33·35·43, §3 #2~5 | N 3장 |
...
| D-G | **홈 A 보완** — 카드의 "자세히" 진입점, 시작 전 카드(흐림), 마친 카드, 조 카드 넷째 줄, 연결 없음 한 줄, 음성 없음 안내, 설정·소개 (i) 우상단, 빈 홈. 초대 코드 입력은 Q-02 답대로(추천: 홈 위 시트, 여섯 칸 UI 유지). L의 초대 코드 진입 삭제 | FR-36·37·40·42, §3 #6~9 | A 상태별 4~5장 + 코드 시트 |
| D-H | **D 여정 상세 보완** — 오늘 처음부터(S3), 조 영역(초대 코드 나누기 · 조원 N/5 · 오늘 배정), 삭제 문구 셋 분기(S6). "함께 바치기 배정" 화면은 Q-08 답대로(추천: 만들지 않음) | FR-18·40·44, §3 #10·15 | D 2장(혼자/조) |
...
| 시안 확정·보완 | `kimdesigner` | §8 과제 → 화면 여덟 + 시트 일곱 × 두 벌, 24항목 자가 점검. 구현 뒤에는 실제 렌더 스크린샷을 눈으로 보는 시각 반복 루프 | 관문 2, 이후 각 M의 화면 검수 |
...
| **두 벌·두 플랫폼·여덟 화면**으로 늘어난 표면 | 일정 | M2에서 밤 벌은 토큰 교체로 얻도록 구조를 잡는다(06-b가 이미 CSS 변수 구조) |
...
| # | 질문 | 추천 | 답 |
|---|---|---|---|
| 1 | **결정 0 네 항목**이 맞나 — 이 저장소에서 개발 · 정본 이관과 atelier freeze · **Android를 V1에 포함(PRD §6 개정)** · 기획 컨펌 → 디자인 컨펌 → 개발 순서 | 네 항목 그대로 | ☐ 그대로 / ☐ 고칠 것: |
| 2 | **프레임워크·스택** (§7 카드) — A안 Expo + EAS Build + Supabase로 가나 | **A안** | ☐ A / ☐ B / ☐ C / ☐ D / ☐ 보류 |
| 3 | **초대 코드 입력의 자리**(Q-02) — 프로토타입의 별도 화면인가, PRD대로 홈 위 시트인가 | **홈 위 시트** (여섯 칸 UI는 살림) | ☐ 시트 / ☐ 별도 화면 |
| 4 | **새 기도(N)에 낭송 방식을 인라인으로 두나**(Q-07) — 프로토타입은 인라인, PRD는 설정에서만 | **설정에서만** (PRD) | ☐ 설정에서만 / ☐ N 인라인 |
| 5 | **"함께 바치기 배정" 화면과 "다섯 단 모두 혼자 바치기" 단추**(Q-08)를 만드나 | **만들지 않음** (홈 카드·상세 D에 정보 있음) | ☐ 만들지 않음 / ☐ 만듦 |

이 다섯에 답하면 관문 1이 닫히고, 답을 `decisions.md`에 적은 뒤 KimDesigner가 §8 과제로 관문 2를 시작한다. **함께 시작해 두면 좋은 사람 몫**: §9-1의 스토어 계정 둘(승인에 며칠 걸린다).

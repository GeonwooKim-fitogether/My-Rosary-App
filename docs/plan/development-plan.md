...
| 19 | 터치 영역 | 80px 실측, 88px 권고 | §6·06-b §7 "최소 80픽셀" | **80px 유지** — 88 은 기기 픽셀 환산 착오였다 (Q-16 이 Q-06b 를 대체) |
...
| 목표 | 성공 기준 | 언제 |
|---|---|---|
| **목표 1 — 08 검증 진입** | 공방장 폰(iOS)에 TestFlight로, Android 검증자 폰에 Google Play **비공개 테스트**로 설치된 앱이 있고, **혼자 54일 여정을 만들어 첫날 77단계를 손 없이 완주**할 수 있으며(중심 질문 DQ-05), 계정 로그인·기기 간 이어가기·조 기도가 동작한다. 검증 참가자 **12~15명**이 4주 쓴다 — PRD가 적은 "10명 이내"에서 늘렸다(Q-34) | M4 끝 |
...
flowchart LR
    ...
    M3 --> M4["M4 08 검증 진입<br/>TestFlight·비공개 테스트 배포"]
...
§13 체크리스트의 다섯 항목이 모두 답을 받았다. 화면이 여덟에서 열로 늘어난 것이 이 관문의 가장 큰 산출이다. PRD §6·§9·터치 영역·치자-d 개정은 2026-09-08 에 `docs/product/06-prd.md` 와 `docs/product/06-design-system.md` 양쪽에 반영을 마쳤다(경위는 `decisions.md` 결정 0 아래의 반영 기록).
...
| **M4 08 검증 진입** | 검증 빌드(S7 소개 시트 포함), TestFlight **외부 테스트**와 Google Play **비공개 테스트** 트랙에 신자 **12~15명**(Q-34), 참가자 안내문, 지표 수집 방식(PRD §7), D-4 공식 기도문 전문 입력 | (1) 참가자 폰에 설치되고 첫날 완주 보고, (2) 4주 뒤 지표 두 줄 | 참가자 모집, 기도문 사용 권한 확인(D-4), 사제 자문 |
...
| Google Play Console 등록 | 브라우저 → play.google.com/console → 계정 만들기 (개인, 1회 $25) | Google 계정으로 로그인 → 개발자 계정 만들기 → 본인 확인 → 결제 | Console 홈에 "앱 만들기" 단추가 보인다 | M0의 Android 테스트 배포. **주의**: 2023-11 이후에 만든 신규 개인 계정은 정식 출시 권한을 신청하기 전에 **비공개 테스트(closed testing)를 테스터 12명 이상이 14일 연속 참여한 상태로** 운영해야 하고, 신청 시점에도 12명이 참여 중이어야 한다. 요건은 계정마다가 아니라 **앱마다** 적용된다 — 08 검증 참가자가 이 요건을 겸하도록 M4에서 계획한다(Q-34) |
| 빌드 서비스 계정 (A안이면 Expo 계정) | 브라우저 → expo.dev 가입 | 무료 계정 생성. 저장소 연결은 Claude가 안내 | 대시보드에 프로젝트가 보인다 | M0의 클라우드 빌드 |

### 9-2. 08 검증 배포 (M4)

- iOS: TestFlight 내부 테스트(공방장) → 외부 테스트(참가자, 애플의 간이 심사 통과 필요). 애플에는 인원 요건이 없다.
- Android: Google Play **비공개 테스트(closed testing) 트랙**에 참가자 이메일을 등록한다. **내부 테스트 트랙이 아니다** — 내부 테스트는 초기 점검용 별개 트랙이라 정식 출시 요건을 채우지 못한다. 이 트랙에 12명 이상이 14일 연속 참여하면 4주 검증 기간이 요건을 그대로 덮는다(Q-34).
...
| **Google Play 신규 개인 계정의 비공개 테스트 요건**(12명 × 14일 연속) | M5 일정 | 08 검증 참가자를 **비공개 테스트 트랙**에 등록해 요건을 겸한다. 4주 검증이 14일을 덮으므로 추가 기간이 들지 않는다. 참가자가 12명 미만으로 떨어지면 신청이 막히므로 여유를 두어 12~15명으로 모집한다(Q-34) |
...
| Q-34 | 이 문서가 적었던 구글 테스트 요건(20명 · 내부 테스트 트랙)이 실제 정책과 어긋났다 | **12명 × 14일 연속 · 비공개 테스트 트랙**이 실제 요건이다. 검증 참가자를 **12~15명**으로 늘려 4주 검증이 요건을 겸하게 한다 — 추가 일정 0. 모집 시작 전에 확정한다 |
